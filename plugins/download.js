...
cmd({
  ...
  try {
    ...
    const response = await axios.get(`https://api.botcahx.eu.org/api/dowloader/igdowloader?url=${args[0]}&apikey=${btc}`);
...
cmd({
  ...
  try {
    ...
    const response = await axios.get(`https://api.botcahx.eu.org/api/download/twitter2?url=${args[0]}&apikey=${btc}`);
    const data = response.data;
        ...
        switch (receivedText) {
          ...
          case "4":
            await conn.sendMessage(senderID, {
              ...
              caption: "📥 *Audio Downloaded as Document by Joshuamambo1*"
            }, { quoted: receivedMsg });
            break;
...
╰━━━━━━━━━━━━━━━┈⊷
🔗 **© 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 MEGATRONE-MDмυℓтιρℓє ρσωєяƒυℓ ωнαтѕαρρ вσт ❣️*`;
...
cmd({
  ...
  try {
    ...
    const apiUrl = `https://api.botcahx.eu.org/api/download/gdrive?url=${text}&apikey=${btc}`;
    const response = await axios.get(apiUrl);
    ...
    if (downloadUrl) {
      ...
      await conn.sendMessage(from, {
        ...
        caption: "*© 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 MEGATRONE-MD ❣️*"
      }, { quoted: m });
